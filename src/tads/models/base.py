...
def validate_training_data_temporal_bounds(data: pa.Table, max_allowed_timestamp: datetime) -> None:
    """
    ...
    if max_ts_in_data.tzinfo is None:
        max_ts_in_data = max_ts_in_data.replace(tzinfo=UTC)
...
class BaseModel:
    ...
    def __init__(self, training_end_bound: datetime) -> None:
